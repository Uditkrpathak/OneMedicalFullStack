  ...
  try {
    ...
    const formattedQueue = appointments.map(appt => {
      ...
      return {
        ...
        appointmentPlace: appt.appointmentPlace || (isTelehealth ? 'VIDEO' : 'CLINIC'),
        patientAddressSnapshot: appt.patientAddressSnapshot,
        patientAddress: appt.patientAddress || appt.homeAddress,
        paymentStatus: appt.paymentStatus || 'PENDING',
        amount: appt.amount || 0,
        status: appt.status || 'CONFIRMED',
        sessionStatus: appt.sessionStatus || 'NOT_STARTED',
        attendanceOutcome: appt.attendanceOutcome || null,
        consultationType: isTelehealth ? 'VIDEO' : (appt.appointmentPlace === 'HOME' ? 'HOME' : 'CLINIC'),
        type: isTelehealth ? 'telehealth' : (appt.appointmentPlace === 'HOME' ? 'home_visit' : 'clinic_visit'),
